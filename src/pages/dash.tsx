...
function QuickStatCard({
  ...
  return (
    <Card className="group relative overflow-hidden border-white/[0.06] bg-white/[0.02] backdrop-blur-xl hover:bg-white/[0.04] transition-all duration-500 hover:border-white/[0.12] hover:shadow-lg hover:shadow-black/20">
...
function TrendingTicker({
  ...
  return (
    <button
      onClick={onClick}
      className="group flex items-center gap-3 rounded-xl border border-white/[0.04] bg-white/[0.02] px-4 py-2.5 transition-all hover:bg-white/[0.06] hover:border-white/[0.08] cursor-pointer shrink-0"
    >
      <span className="text-sm font-semibold text-zinc-200 group-hover:text-white transition-colors">{symbol}</span>
      {loading ? (
        <>
          <Skeleton className="h-3.5 w-14 rounded bg-white/[0.06]" />
          <Skeleton className="h-3.5 w-10 rounded bg-white/[0.06]" />
        </>
      ) : (
        <>
...
function NavItem({
  ...
  return (
    ...
    >
      <Icon className={`h-[18px] w-[18px] shrink-0 ${active ? "text-white" : "text-zinc-600 group-hover:text-zinc-400"} transition-colors`} />
...
function MarketPulseItem({ article }: { article: NewsArticle }) {
  ...
  return (
    <Wrapper {...wrapperProps} className="group flex items-start gap-3 rounded-xl p-3 transition-all hover:bg-white/[0.03] cursor-pointer">
      <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-white/[0.04] ring-1 ring-white/[0.06] mt-0.5">
        <Zap className="h-3.5 w-3.5 text-amber-400" />
      </div>
      <div className="flex-1 min-w-0">
        <p className="text-[13px] font-medium text-zinc-300 leading-snug line-clamp-2 group-hover:text-white transition-colors">{article.title}</p>
        <div className="flex items-center gap-2 mt-1.5">
          <span className="text-[10px] text-zinc-600">{timeAgo(article.timestamp)}</span>
          <Badge variant="outline" className="h-4 border-white/[0.06] bg-white/[0.02] text-[9px] font-medium text-zinc-500 px-1.5">
            {tag}
          </Badge>
          <span className="text-[9px] text-zinc-700 truncate max-w-[80px]">{article.publisher}</span>
        </div>
      </div>
    </Wrapper>
  )
      ...
      {Array.from({ length: 5 }).map((_, i) => (
        <div key={i} className="flex items-start gap-3 p-3 animate-pulse">
          ...
          <div className="flex-1 space-y-2 pt-0.5">
            ...
            <div className="h-2 bg-white/[0.04] rounded w-1/3 mt-1" />
          </div>
        </div>
      ))}
    </div>
...
export default function Page() {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-[#09090b] text-white">
      {/* ─── SIDEBAR ─── */}
      <aside className={`hidden lg:flex flex-col border-r border-white/[0.04] bg-[#09090b] transition-all duration-300 ease-in-out ${sidebarCollapsed ? "w-[72px]" : "w-[260px]"}`}>
        {/* Logo */}
        <div className={`flex h-16 items-center border-b border-white/[0.04] ${sidebarCollapsed ? "justify-center px-2" : "gap-3 px-5"}`}>
          ...
          {!sidebarCollapsed && <span className="text-[15px] font-bold tracking-tight text-white">Fintola</span>}
        </div>
        ...
        {/* Collapse toggle */}
        <div className={`border-t border-white/[0.04] ${sidebarCollapsed ? "px-2 py-2" : "px-3 py-2"}`}>
          <button
            title={sidebarCollapsed ? "Expand sidebar" : "Collapse sidebar"}
            onClick={() => setSidebarCollapsed((prev) => !prev)}
            className="flex w-full items-center justify-center gap-2 rounded-xl px-2 py-2 text-zinc-600 hover:text-zinc-300 hover:bg-white/[0.04] transition-all"
          >
      ...
      <main className="flex-1 overflow-y-auto">
        ...
        <div className="p-6 space-y-6 max-w-[1600px] mx-auto">
          ...
          <div className="flex items-center gap-2 overflow-x-auto pb-1 scrollbar-hide">
            <div className="flex h-7 items-center gap-1.5 rounded-lg bg-white/[0.03] px-2.5 shrink-0 ring-1 ring-white/[0.06]">
          ...
          <div className="grid grid-cols-1 xl:grid-cols-[1fr_340px] gap-6 items-start">
            {/* Chart */}
            <Card className="overflow-hidden border-white/[0.06] bg-white/[0.02] backdrop-blur-xl min-w-0">
              ...
              <Card className="border-white/[0.06] bg-white/[0.02] backdrop-blur-xl overflow-hidden">
                ...
                <CardContent className="px-5 pb-5">
                  <div className="grid grid-cols-2 gap-2">
                    <button onClick={() => setSelectedSymbol("^NSEI")} className="flex flex-col items-center gap-1.5 rounded-xl border border-white/[0.04] bg-white/[0.02] p-3 hover:bg-white/[0.06] hover:border-white/[0.08] transition-all">
                      <TrendingUp className="h-4 w-4 text-emerald-400" />
                      <span className="text-[11px] font-medium text-zinc-400">Nifty 50</span>
                    </button>
                    <button onClick={() => setSelectedSymbol("BTC-USD")} className="flex flex-col items-center gap-1.5 rounded-xl border border-white/[0.04] bg-white/[0.02] p-3 hover:bg-white/[0.06] hover:border-white/[0.08] transition-all">
                      <Globe className="h-4 w-4 text-orange-400" />
                      <span className="text-[11px] font-medium text-zinc-400">Bitcoin</span>
                    </button>
                    <button onClick={() => setSelectedSymbol("RELIANCE.NS")} className="flex flex-col items-center gap-1.5 rounded-xl border border-white/[0.04] bg-white/[0.02] p-3 hover:bg-white/[0.06] hover:border-white/[0.08] transition-all">
                      <Briefcase className="h-4 w-4 text-indigo-400" />
                      <span className="text-[11px] font-medium text-zinc-400">Reliance</span>
                    </button>
                    <button onClick={() => router.push("/trade")} className="flex flex-col items-center gap-1.5 rounded-xl border border-white/[0.04] bg-white/[0.02] p-3 hover:bg-white/[0.06] hover:border-white/[0.08] transition-all">
                      <BarChart3 className="h-4 w-4 text-violet-400" />
                      <span className="text-[11px] font-medium text-zinc-400">Trade</span>
                    </button>
                  </div>
                </CardContent>
              </Card>
            ...
            <CardHeader className="px-5 pt-5 pb-3">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <CardTitle className="text-base font-semibold text-white">Indian Market Overview</CardTitle>
                    <p className="text-[11px] text-zinc-600 mt-0.5">Live prices · NSE &amp; BSE</p>
                  </div>
                </div>
                <Tabs defaultValue="all" className="w-auto">
                  <TabsList className="h-8 bg-white/[0.04] border border-white/[0.06] rounded-lg p-0.5">
                    <TabsTrigger value="all" className="text-[11px] h-7 px-3 rounded-md data-[state=active]:bg-white/[0.08] data-[state=active]:text-white text-zinc-500">All</TabsTrigger>
                    <TabsTrigger value="stocks" className="text-[11px] h-7 px-3 rounded-md data-[state=active]:bg-white/[0.08] data-[state=active]:text-white text-zinc-500">Stocks</TabsTrigger>
                    <TabsTrigger value="indices" className="text-[11px] h-7 px-3 rounded-md data-[state=active]:bg-white/[0.08] data-[state=active]:text-white text-zinc-500">Indices</TabsTrigger>
                  </TabsList>
                </Tabs>
              </div>
            </CardHeader>
